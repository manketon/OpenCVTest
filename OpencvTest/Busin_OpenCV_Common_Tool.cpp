...
CBusin_OpenCV_Common_Tool& CBusin_OpenCV_Common_Tool::instance()
{
...
int CBusin_OpenCV_Common_Tool::test_dilate(const string& str_img_path)
{
...
int CBusin_OpenCV_Common_Tool::test_shrink(const string& str_img_path)
{
...
int CBusin_OpenCV_Common_Tool::test_resize(const string& str_img_path)
{
	int nResize_height = 200;
	int nResize_width = 200;
...
void CBusin_OpenCV_Common_Tool::scaleIntervalSampling(const Mat &src, Mat &dst, double xRatio, double yRatio)
{
	//只处理uchar型的像素
...
void CBusin_OpenCV_Common_Tool::scalePartAverage(const Mat &src, Mat &dst, double xRatio, double yRatio)
{
	int rows = static_cast<int>(src.rows * yRatio);
	int cols = static_cast<int>(src.cols * xRatio);
...
void CBusin_OpenCV_Common_Tool::average(const Mat &img, Point_<int> a, Point_<int> b, Vec3b &p)
{
	const Vec3b *pix;
	Vec3i temp;
...
void CBusin_OpenCV_Common_Tool::shrink_by_part_average(const Mat &src, Mat &dst, double xRatio, double yRatio)
{
	//只处理uchar型的像素
	CV_Assert(src.depth() == CV_8U);
	int rows = static_cast<int>(src.rows * yRatio);
...
void CBusin_OpenCV_Common_Tool::average_for_3_channel(const Mat &img, const Point& a, const Point& b, Vec3b &p)
{
	const Vec3b *pix;
	Vec3i temp;
...
void CBusin_OpenCV_Common_Tool::average_for_1_channel(const Mat &img, const Point& a, const Point& b, uchar &nAverage)
{
...
int CBusin_OpenCV_Common_Tool::detect_circles(const Mat& src_bgr_mat, const Rect& roi, double dMin_centers_dist, vector<Vec3f>& circles, double dMax_canny_threshold /*= 100*/, double dCircle_center_threshold /*= 100*/,int minRadius /*= 0*/, int maxRadius /*= 0*/)
{
	try
	{
		Mat mat_gray;
...
int CBusin_OpenCV_Common_Tool::test_detect_circles(const std::string& str_img_path)
{
...
int CBusin_OpenCV_Common_Tool::test_transform()
{
...
double CBusin_OpenCV_Common_Tool::get_euclidean(const cv::Point& pointO, const cv::Point& pointA)
{
	double distance;  
...
int CBusin_OpenCV_Common_Tool::deal_nc_point(const Point& point_00_before, const Point& point_00_after , const Point2f& point_src_from_nc, const cv::Mat& warp_mat, const float& fScale_pic_devide_mm_before , const float& fScale_pic_devide_mm_after, Point2f& point_dst_to_nc, std::string& str_kernel_err_reason)
{
...
cv::Point2f CBusin_OpenCV_Common_Tool::get_dst_point_after_affine(const cv::Point2f& src_point, const Mat& affine_transform_mat)
{
...
bool CBusin_OpenCV_Common_Tool::is_special_right_triangle(const cv::Point& point_A, const cv::Point& point_B , const cv::Point& point_C, const double& dBias_degree)
{
...
void CBusin_OpenCV_Common_Tool::callback_change_contrast_and_brightness(int pos, void* userdata)
{
	printf("%s | enter\n", __FUNCTION__);
...
int CBusin_OpenCV_Common_Tool::test_change_contrast_and_brightness(const string& str_img_path)
{
	int nContrast = 1;
	int nBrightness = 13; 
	/// 创建显示窗口
...
int CBusin_OpenCV_Common_Tool::test_wenzi_G_code(const string& str_img_path)
{
	try
	{
		/// 装载图像
...
int CBusin_OpenCV_Common_Tool::test_shrink_mat()
{
	/// 装载图像
...
int CBusin_OpenCV_Common_Tool::rotate_and_shrink_binary_writting_mat(const Mat& mat_src_binary_gray, const float& fMax_width_pxl , const float& fMax_height_pxl, const float& fScale_pic_devide_mm, const Point& point_00, Mat& mat_binary_gray_shrinked)
{
	int ret = 0;
...
int CBusin_OpenCV_Common_Tool::shrink_binary_mat_using_contours(const Mat& mat_src_binary, const float& fMax_width_pxl , const float& fMax_height_pxl, const Point& point_00, Mat& mat_binary_shrinked)
{
...
cv::Mat CBusin_OpenCV_Common_Tool::override_sub_mat(const Mat& m1, const Mat& m2, const Point& rio_loction_point)
{
...
int CBusin_OpenCV_Common_Tool::test_override_sub_mat()
{
	//设置深蓝色背景画布
...
int CBusin_OpenCV_Common_Tool::remove_error_black_points(Mat& mat_binary)
{
	size_t nStep = 5;
...
int CBusin_OpenCV_Common_Tool::calc_black_points_num(const Mat& mat_binary, const Point& point_current, size_t nStep)
{
...
int CBusin_OpenCV_Common_Tool::draw_lines(const Mat& mat_src, Point2f* pArr, size_t n, const string& str_img_path)
{
...
int CBusin_OpenCV_Common_Tool::draw_lines(const Mat& mat_src, const Point2f* pArr, size_t n, const Scalar& color, const string& str_win_name)
{
	Mat mat_tmp = mat_src.clone();
	for (int i = 0; i != n; ++i)
	{
...
int CBusin_OpenCV_Common_Tool::test_get_binary_gray_mat(const string& str_images_dir)
{
	//获取指定目录中的所有图片
...
int CBusin_OpenCV_Common_Tool::rotate_writting_to_horizontal_direction(const Mat& src_mat, const RotatedRect& rRect , const float& fScale, const float& fScale_pic_devide_mm, Mat& mat_dst)
{
	//获取变换前的最小外接矩形的四个顶点
...
int CBusin_OpenCV_Common_Tool::get_points_after_affined(const Mat& mat_rotation, const Point2f* pSrc_points, size_t nPoints_num, Point2f* pDst_points)
{
...
int CBusin_OpenCV_Common_Tool::get_rect_points(const Mat& mat_src, const RotatedRect& rRect, Point2f* pts, size_t nPoints_num /*= 4*/)
{
...
int CBusin_OpenCV_Common_Tool::sort_rect_points(Point2f* pRect_points, size_t nPoints_num)
{
...
int CBusin_OpenCV_Common_Tool::increase_rect_size(Point2f* pRect_points, size_t nPoints_num, int nExt_size_pxl)
{
	//bottomLeft的x变小且y变大
...
void CBusin_OpenCV_Common_Tool::get_connected_contours(const Mat& mat_src_binary_gray, vector<vector<Point> >& vec_contours)
{
...
void CBusin_OpenCV_Common_Tool::traverse_connected_graph(Mat& mat_src_binary_gray, const Point& point_start, vector<Point>& vec_points)
{
...
int CBusin_OpenCV_Common_Tool::get_binary_gray_mat(const cv::Mat& mat_src, int nThreshold_binary, Mat& mat_dst_binary)
{
	Mat mat_src_gray;
	// 		/// 原图像转换为灰度图像
...
int CBusin_OpenCV_Common_Tool::create_nc_code(const Mat& mat_dst_binary, const Point& point_00 , float fScale_pic_devide_mm, float fZ_up, float fZ_down, string& str_nc_data)
{
	size_t nLastN = 0;
...
int CBusin_OpenCV_Common_Tool::get_nc_effective_data(const vector<vector<Point> >& vec_contours, const Point& point_00, size_t& nLastN , float fScale_pic_devide_mm, float fZ_up, float fZ_down, string& str_nc_data)
{
...
int CBusin_OpenCV_Common_Tool::get_nc_effective_data(const Mat& mat_binary, const Point& point_00, size_t& nLastN , float fScale_pic_devide_mm, float fZ_up, float fZ_down, string& str_nc_data)
{
	bool bIs_first_black = false; //第一个黑点为第一行有效数字，即N9
	bool bIs_lifting_knife = true; //遍历某列时,雕刻刀的状态，如果雕刻刀是抬刀状态，则在雕刻某点事，则在此点应该先抬刀再雕刻。
...
string CBusin_OpenCV_Common_Tool::get_nc_head(size_t& nLastN)
{
